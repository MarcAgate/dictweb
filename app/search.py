...
def get_sort_tibetan_key(entry: dict) -> str:
...
def sort_entries_by_tibetan(entries: list[dict]) -> list[dict]:
    if not entries:
        return []
...
def build_entries_from_rows(rows) -> list[dict]:
    by_wylie = {}
...
def build_tabs_for_wylie(rows, selected_wylie: str) -> dict:
    tabs = {
        "fr": [],
        "eng": [],
        "tib": []
